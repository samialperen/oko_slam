...
def callback_laser(msg):
	...
	'north-east': min(min(msg.ranges[6:10]), MAX_LIDAR_RANGE)
}
...
def take_action():
	...
	max_dist2robot = 0.20 #If detected object is far away from this value, we 
	# don't consider it
	max_distance2object = 0.20 
	# If there exists an object only north then turn left --> Case 2
	# Positive turn around z axis corresponds to turning left
	
	if regions['north-west'] < max_distance2object and regions['north-east'] < max_distance2object:
		state_description = 'case 9 - north-west and north-east'
		change_state(3)
	elif regions['north-west'] < max_distance2object and regions['north-east'] > max_distance2object:
		state_description = 'case 10 - north-west'
		change_state(3)
	elif regions['north-west'] > max_distance2object and regions['north-east'] < max_distance2object :
		state_description = 'case 11 - north-east'
		change_state(1)
	else:
