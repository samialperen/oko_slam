...
int main(int argc, char** argv)
{
  ...
  std::string read_dir;
  std::string obj_name;
  parser::ParseArg opts;
  parser::ParseArg::statusReturn_e temp = opts.parse(argc, argv);
  if ( parser::ParseArg::OPTS_SUCCESS == temp )
  {
  	read_dir = opts.getSaveDir();
  	obj_name = opts.getObjName();
  }
  else if( parser::ParseArg::OPTS_HELP == temp ) {}
  else
  {
